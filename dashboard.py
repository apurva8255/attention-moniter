...
class ConfusionMatrixChart:
    ...
    def draw(self, cm, model_name: str):
        """Draw confusion matrix given numpy array or list of lists cm."""
        if not cm:
            return
        ...
        # Plot using sklearn's display but target our axis
        disp = ConfusionMatrixDisplay(confusion_matrix=cm_np, display_labels=labels)
        disp.plot(ax=self.ax, cmap="Blues", colorbar=False)

        # Style colors for dark theme readability
        for text in disp.text_.ravel():
            text.set_color(TEXT_COLOR)

        self.ax.set_title(f"Confusion Matrix — {model_name}", color=TEXT_COLOR,
                          fontsize=10, fontweight="bold")
        self.ax.tick_params(colors=TEXT_COLOR, labelsize=8)
